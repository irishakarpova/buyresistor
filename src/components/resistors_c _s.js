...
function Resistors_k({ handleAddToCart, categories }) {
  ...
  const fetchProducts = async () => {
    const { data } = await commerce.products.list({
      category_slug: ['1-8w-carbon-film-resistors'],
      limit: 200,
    });
    setProducts(data);
    setLoading(false);
  ...
  const getListOfProducts = () => {
    return products.map((product) => {
      return (
        <div key={product.id}>
          <CardResistor handleAddToCart={handleAddToCart} resistor={product} />
        </div>
      );
    });
  };
        ...
        <Box>
              ...
              ? categories.map((item) => {
                  return (
                    <Chip
                      ...
                      href={item.slug}
                      variant={
                        item.slug === '1-8w-carbon-film-resistors'
                          ? 'filled'
                          : 'outlined'
                      }
                    />
          ...
          {!loading ? (
            <Typography variant="h5">
              Pack (10 pcs) of 1/8W Carbon Film Resistors
            </Typography>
          ) : (
            <Box
              sx={{
